...
async function getAllShotPowerScores() {
  try {
    // Get all players
    ...
    for (const evaluation of evaluationsResult.rows) {
      ...
      for (const [playerId, playerScores] of Object.entries(scores)) {
        if (playersMap.has(playerId)) {
          ...
          // Extract shot power scores
          const shotPowerData = {
            evaluationId: evaluation.id,
            evaluationName: evaluation.name,
            teamId: evaluation.team_id,
            strongFoot: {
              attempt1: playerScores.power_strong_1 || null,
              attempt2: playerScores.power_strong_2 || null,
              attempt3: playerScores.power_strong_3 || null,
              attempt4: playerScores.power_strong_4 || null,
            },
            weakFoot: {
              attempt1: playerScores.power_weak_1 || null,
              attempt2: playerScores.power_weak_2 || null,
              attempt3: playerScores.power_weak_3 || null,
              attempt4: playerScores.power_weak_4 || null,
            },
          };

          // Calculate averages and max
          const strongAttempts = [
            shotPowerData.strongFoot.attempt1,
            shotPowerData.strongFoot.attempt2,
            shotPowerData.strongFoot.attempt3,
            shotPowerData.strongFoot.attempt4,
          ].filter((v) => v !== null && v !== undefined);

          const weakAttempts = [
            shotPowerData.weakFoot.attempt1,
            shotPowerData.weakFoot.attempt2,
            shotPowerData.weakFoot.attempt3,
            shotPowerData.weakFoot.attempt4,
          ].filter((v) => v !== null && v !== undefined);

          if (strongAttempts.length > 0) {
            shotPowerData.strongFoot.avg =
              strongAttempts.reduce((a, b) => a + b, 0) / strongAttempts.length;
            shotPowerData.strongFoot.max = Math.max(...strongAttempts);
          }

          if (weakAttempts.length > 0) {
            shotPowerData.weakFoot.avg =
              weakAttempts.reduce((a, b) => a + b, 0) / weakAttempts.length;
            shotPowerData.weakFoot.max = Math.max(...weakAttempts);
          }

          player.scores.push(shotPowerData);
        }
      }
    }
    ...
    // Output results
    console.log("\n" + "=".repeat(100));
    console.log("ALL PLAYERS - SHOT POWER SCORES (CYCLE 1 ONLY)");
    ...
    console.log(`  Players with Cycle 1 Shot Power Scores: ${totalPlayersWithCycle1}`);
    console.log("=".repeat(100));
