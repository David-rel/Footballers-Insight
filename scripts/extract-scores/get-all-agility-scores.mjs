...
async function getAllAgilityScores() {
  try {
      ...
      for (const [playerId, playerScores] of Object.entries(scores)) {
        if (playersMap.has(playerId)) {
          ...
          const agilityData = {
            ...
            trials: [
              playerScores.agility_1 || null,
              playerScores.agility_2 || null,
              playerScores.agility_3 || null,
            ],
          };

          const validTrials = agilityData.trials.filter(t => t !== null && t !== undefined);
          if (validTrials.length > 0) {
            agilityData.average = validTrials.reduce((a, b) => a + b, 0) / validTrials.length;
            agilityData.best = Math.min(...validTrials); // Lower is better for time
          }

          player.scores.push(agilityData);
        }
      }
    }

    console.log("\n" + "=".repeat(100));
    console.log("ALL PLAYERS - AGILITY (5-10-5) SCORES (CYCLE 1 ONLY)");
    ...
    for (const player of playersMap.values()) {
      ...
      for (const scoreData of cycle1Scores) {
        ...
        for (let i = 0; i < 3; i++) {
          console.log(`  Trial ${i + 1}: ${scoreData.trials[i] ?? "—"} seconds`);
        }
        if (scoreData.average !== undefined) {
          console.log(`  Average: ${scoreData.average.toFixed(2)} seconds`);
        }
        if (scoreData.best !== undefined) {
          console.log(`  Best: ${scoreData.best.toFixed(2)} seconds`);
        }
        console.log("");
      }
      console.log("");
    ...
    console.log(`  Players with Cycle 1 Agility Scores: ${totalPlayersWithCycle1}`);
    console.log("=".repeat(100));
    ...
    console.log("=".repeat(100));
    console.log(
      "Player Name,Evaluation Name,Trial 1,Trial 2,Trial 3,Average,Best"
    );
