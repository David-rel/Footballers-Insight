...
async function getAllCorePlankScores() {
  try {
      ...
      for (const [playerId, playerScores] of Object.entries(scores)) {
        if (playersMap.has(playerId)) {
          ...
          const plankData = {
            ...
            time: playerScores.plank_time || null,
            form: playerScores.plank_form !== undefined ? (playerScores.plank_form === 1 ? "OK" : "Broke Form") : null,
          };

          player.scores.push(plankData);
        }
      }
    }

    console.log("\n" + "=".repeat(100));
    console.log("ALL PLAYERS - CORE PLANK SCORES (CYCLE 1 ONLY)");
    ...
    for (const player of playersMap.values()) {
      ...
      for (const scoreData of cycle1Scores) {
        ...
        console.log(`  Time: ${scoreData.time ?? "—"} seconds`);
        console.log(`  Form: ${scoreData.form ?? "—"}`);
        console.log("");
      }
      console.log("");
    }
    ...
    console.log(`  Players with Cycle 1 Core Plank Scores: ${totalPlayersWithCycle1}`);
    console.log("=".repeat(100));
    ...
    console.log("=".repeat(100));
    console.log(
      "Player Name,Evaluation Name,Time (seconds),Form"
    );
