...
// GET - Fetch a specific player evaluation + stats blobs (for comparisons)
export async function GET(
  _request: NextRequest,
  {
    params,
  }: { params: Promise<{ id: string; playerId: string; playerEvaluationId: string }> }
) {
  try {
    ...
    const { id: teamId, playerId, playerEvaluationId } = await params;
    ...
    const peRes = await pool.query(
      `SELECT id, player_id, team_id, evaluation_id, name, created_at
       ...
       WHERE id = $1 AND player_id = $2 AND team_id = $3
       LIMIT 1`,
      [playerEvaluationId, playerId, teamId]
    );
    ...
    return NextResponse.json(
      { error: "Failed to get player evaluation", details: error.message },
      { status: 500 }
    );
  }
}
