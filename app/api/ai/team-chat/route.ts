...
async function authorizeTeamAccess(sessionUserId: string, teamId: string) {
  ...
  return { ok: true as const, userRole, companyId, teamId };
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const authz = await authorizeTeamAccess(session.user.id, teamId);
    if (!authz.ok) {
    ...
    if (threadId) {
      const threadRes = await pool.query(
        `SELECT id, team_id, user_id, messages, last_response_id, updated_at
         FROM team_ai_chat_threads
         WHERE id = $1`,
        [threadId]
      );
      ...
      if (t.user_id !== session.user.id || t.team_id !== teamId) {
    ...
    console.error("AI team chat GET error:", e);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    ...
    const system = [
      ...
      "You are chatting with a COACH about their TEAM and players.",
      "",
      "CRITICAL:",
      "- Soccer only. Do not talk about other sports.",
      "- These are TESTS, not the objective. Treat them as signals of football qualities.",
      "- Do NOT say 'practice the test' or 're-test' or 'do more test reps'.",
      "- Do NOT create drills, activities, sessions, or training plans. Do not ask the coach to run drills.",
      "- If the coach explicitly asks for drills/activities/sessions, reply: 'For specific activities, please use the Activity Generator page — this chat is only for coaching logic and ideas.' Then give brief principles/cues only.",
      "- Stay practical: match relevance, cues, constraints, what to watch for, and how to coach it.",
      "",
      "CHAT MODE:",
      "- Default length: 3–8 short lines OR up to 6 bullets. No essays.",
      "- Ask at most ONE clarifying question when helpful.",
      "- Only go long if explicitly asked for detail.",
    ].join("\n");
    ...
    if (threadId) {
      const threadRes = await pool.query(
        `SELECT id, team_id, user_id, company_id, model, last_response_id, messages
         FROM team_ai_chat_threads
         WHERE id = $1`,
        [threadId]
      );
      ...
      if (!ai.ok) {
        console.error("Team chat OpenAI error:", ai);
        return NextResponse.json({ error: "Failed to chat" }, { status: ai.status ?? 502 });
      }
      ...
      const nextMessages = [
        ...priorMessages,
        { id: `u-${nowIso}`, role: "user", text: message.trim(), createdAt: nowIso },
        { id: `a-${nowIso}`, role: "assistant", text: assistantText, createdAt: nowIso },
      ];

      await pool.query(
        `UPDATE team_ai_chat_threads
      ...
      return NextResponse.json(
        { threadId, assistant: assistantText, messages: nextMessages, hasMemory: !!nextResponseId },
        { status: 200 }
      );
    }
    ...
    const createRes = await pool.query(
      `INSERT INTO team_ai_chat_threads (team_id, company_id, user_id, model, context, messages)
      ...
      [
        ...
        model,
        JSON.stringify(context ?? {}),
        JSON.stringify([{ id: `u-${nowIso}`, role: "user", text: message.trim(), createdAt: nowIso }]),
      ]
    );
    ...
    return NextResponse.json(
      { threadId: newThreadId, assistant: assistantText, messages: firstMessages, hasMemory: !!responseId },
      { status: 200 }
    );
  } catch (e) {
    console.error("AI team chat error:", e);
    return NextResponse.json(
      { error: "Something went wrong. Please try again." },
      { status: 500 }
    );
